...
class PublishService extends repository_1.Repository {
  ...
  async video(options) {
    ...
    await Bluebird.try(() => this.regularVideo(Object.assign({ video: options.video, uploadId }, videoInfo))).catch(
  ...
  async album(options) {
    ...
    for (const item of options.items) {
      if (isPhoto(item)) {
        ...
        await Bluebird.try(() =>
          this.regularVideo(
            Object.assign({ video: item.video, uploadId: item.uploadId, isSidecar: true }, item.videoInfo),
          ),
  ...
  async igtvVideo(options) {
    ...
    const uploadResult = await this.segmentedVideo(
      Object.assign(
        Object.assign(Object.assign({ video: options.video, isIgtvVideo: true }, videoInfo), { uploadId }),
  ...
  async segmentedVideo(options) {
    ...
    const ruploadParams = upload_repository_1.UploadRepository.createVideoRuploadParams(
      options,
      uploadId,
      retryContext,
    );
  ...
  async uploadAndConfigureStoryPhoto(options, configureOptions) {
    ...
    return await this.client.media.configureToStory(
      Object.assign(Object.assign({}, configureOptions), {
        upload_id: uploadId,
        width: imageSize.width,
        height: imageSize.height,
      }),
